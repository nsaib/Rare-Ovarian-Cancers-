...
# --- 7. Read Phosphatase Genes ---

phosphatase_file_path <- "C:/Users/ns01288/OneDrive - University of Surrey/data using disertation/Total_Proteome_wong_et_al_Tarney_et_al.xlsx" # Ensure correct path

phosphatase_data <- read_excel(phosphatase_file_path, sheet = "Phosphatase")

phosphatase_genes <- phosphatase_data$`Entrez Gene Symbol`

# --- 8. Find Overlap with Kinases and Phosphatases ---
...
common_up_phosphatase_genes <- intersect(common_up_genes, phosphatase_genes)
common_down_phosphatase_genes <- intersect(common_down_genes, phosphatase_genes)

# --- 9. Pathway Analysis and Dot Plots ---
...
up_kinase_plot <- pathway_analysis_dotplot(common_up_kinase_genes, "Upregulated Common Kinase Genes")
down_kinase_plot <- pathway_analysis_dotplot(common_down_kinase_genes, "Downregulated Common Kinase Genes")
up_phosphatase_plot <- pathway_analysis_dotplot(common_up_phosphatase_genes, "Upregulated Common Phosphatase Genes")
down_phosphatase_plot <- pathway_analysis_dotplot(common_down_phosphatase_genes, "Downregulated Common Phosphatase Genes")

# --- 10. Combine Plots with Patchwork ---

# Create a list of plots
plots <- list(
  "Upregulated Kinase" = up_kinase_plot,
  "Downregulated Kinase" = down_kinase_plot,
  "Upregulated Phosphatase" = up_phosphatase_plot,
  "Downregulated Phosphatase" = down_phosphatase_plot
)
...
if (length(plots) > 0) {
  combined_plot <- wrap_plots(plots = plots, ncol = 2) + plot_annotation(title = "Pathway Analysis of Kinases and Phosphatases")
  print(combined_plot)
  ggsave("combined_kinase_phosphatase_pathway_analysis.png", combined_plot, width = 12, height = 12)
} else {
